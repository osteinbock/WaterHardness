...
for i = 1:20
    ...
    trainDataZ = (trainData - trainMean) ./ trainStd; 
    % Z-score the test data using the same mean and std from the training data 
    testDataZ = (testData - trainMean) ./ trainStd;  
...
uniqueCategoriesNum = str2double(extractBefore(uniqueCategoriesStr,'ppm')); % Extract numerical part
...
[sortedImportances, sortedIndices] = sort(featureImportances(1,1:46), 'descend');
...
figure;
bar(sortedImportances(1:46));  % Plot the importance values for the top 10 features
title('Top 10 Feature Importances');
xlabel('Metric Name');
ylabel('Importance Score');

% Set x-axis labels to the corresponding feature names
xticks(1:46);  % Set x-tick positions for top 10 features
xticklabels(featureNames(sortedIndices(1:46)));  % Map back to actual feature names
